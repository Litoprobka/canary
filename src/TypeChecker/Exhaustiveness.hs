...
checkCompletePattern :: Ctx es => Located EPattern -> VType -> Eff es ()
checkCompletePattern pat ty = do
    ...
    unless (null result.missing) do
        nonFatal $
            Warn
                Nothing
                "incomplete pattern"
                (mkNotes [(getLoc pat, M.This "this pattern doesn't cover all cases")])
                [Note $ vsep ("cases not covered:" : map (hsep . map pretty) result.missing)]

checkCaseExhaustiveness :: Ctx es => Loc -> VType -> [Located EPattern] -> Eff es ()
...
checkMatchExhaustiveness :: Ctx es => Loc -> NonEmpty VType -> [NonEmpty (Located EPattern)] -> Eff es ()
...
checkExhaustiveness
    :: (Diagnose :> es, State UniVars :> es, State ConstructorTable :> es)
    => NonEmpty VType
    -> [NonEmpty (Located EPattern)]
    -> Eff es CheckResult
checkExhaustiveness types branches = do
    conMap <- get @ConstructorTable
    patternTypes <- traverse simplifyPatternType types
    let branches' = map (\pats -> map (simplifyPattern . unLoc) (toList pats) :@ foldMap1 getLoc pats) branches
...
simplifyPatternType :: forall es. (Diagnose :> es, State UniVars :> es) => VType -> Eff es ExType
